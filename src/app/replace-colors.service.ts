...
export class ReplaceColorsService {
  set original(originalColors: string[]) {
    this._replacement = new Map(
      originalColors.map((originalColor) => [
        originalColor,
  ...
  async replace(file: File) {
    ...
    this._replacement.forEach(
      (value, key) => (content = content.replace(key, value)),
    )
  ...
  update(original: string, replaced: string) {
    ...
    if (this._replacement.get(original) === replaced) return
